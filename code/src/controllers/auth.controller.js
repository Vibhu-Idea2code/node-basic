...
const register = async (req, res) => {
  ...
  const data = await authService.createUser(filter);
...
const login = async (req, res) => {
  try {
    ...
    let data;
    if (token) {
      data = await authService.findUserAndUpdate(findUser._id, token);
    }
...
const getAllUser = async (req, res) => {
  try {
    ...
    const data = await authService.getAllUser({ role: "admin" });
    res.status(200).json({ data });
  } catch (error) {
    res.status(404).json({ error: error.message });
  }
